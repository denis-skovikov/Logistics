...
Сервис управления бронированием слотов с ограниченной вместимостью. Слот — это временное окно с фиксированным числом мест: складское окно приёмки, интервал доставки или время приёма клиентов.

Пользователь сначала создаёт временный **холд** (бронь на 5 минут), а затем подтверждает его. Если не подтвердить вовремя — холд автоматически освобождается, и место возвращается в доступ.

Основная задача сервиса — корректно работать под высокой нагрузкой:

- **Защита от оверсела** — нельзя забронировать больше мест, чем есть, даже при одновременных запросах. Реализовано через транзакции с блокировками на уровне БД.
- **Горячий кеш** — список слотов кешируется в Redis на 15 секунд, с защитой от cache stampede (одновременного наплыва запросов при пустом кеше).
- **Идемпотентность** — повторный запрос на бронь с тем же ключом не создаёт дубль.

В проекте реализованы **два взаимозаменяемых подхода** к подсчёту свободных мест (переключаются через ENV):

- **Denormalized** — остаток хранится в отдельном поле `remaining` (быстрое чтение).
- **Dynamic** — остаток вычисляется на лету через JOIN с таблицей холдов (всегда консистентен).

### Пример использования

Представим службу доставки с ограниченным числом окон на день. Слот «14:00–16:00» имеет вместимость 10 заказов.

1. Клиент выбирает окно доставки → сервис создаёт **холд** на 5 минут (место 6 из 10 занято временно), пока клиент оформляет заказ.
2. Клиент подтверждает заказ → холд переходит в статус **confirmed**, место закреплено окончательно.
3. Если клиент передумал или завис на 5 минут → холд **автоматически отменяется**, место возвращается другим клиентам.

При этом если 10 клиентов одновременно попытаются занять последнее место — только один получит бронь, остальные увидят «мест нет» (защита от оверсела).
